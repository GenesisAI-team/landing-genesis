...
import { ScanEye, Code2, Rocket, ChevronsDown } from 'lucide-react';
...
const steps = [
  {
    ...
    desc: "Auditamos tus flujos de trabajo actuales para detectar cuellos de botella e ineficiencias ocultas.",
    icon: ScanEye,
    // Original Multi-color Theme
  ...
  },
  {
    id: 2,
    title: "CONSTRUCCIÓN",
    subtitle: "DESARROLLO SEGURO",
    desc: "Diseñamos e implementamos tus agentes de IA personalizados en un entorno de pruebas aislado.",
    icon: Code2,
    // Original Multi-color Theme
  ...
  },
  {
    ...
    desc: "Entrega llave en mano, formación exhaustiva a tu equipo y soporte técnico continuo.",
    icon: Rocket,
    // Original Multi-color Theme
...
const ProcessSection: React.FC = () => {
  return (
    <section id="metodo" className="relative w-full py-24 md:py-32 bg-slate-900 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="text-center mb-20 md:mb-32">
                <div className="flex items-center justify-center gap-3 mb-4 opacity-90">
                    ...
                    <h3 className="font-mono text-sm md:text-base text-neon tracking-[0.3em] uppercase font-bold">EL PROTOCOLO GÉNESIS</h3>
                    <div className="w-1.5 h-1.5 bg-neon rounded-full animate-pulse"></div>
                </div>
                    ...
                    {steps.map((step, index) => (
                        <div key={index} className="relative flex md:flex-col items-start md:items-center group">
                            ...
                            <div className="flex-shrink-0 relative z-10 mr-6 md:mr-0 md:mb-10">
                                ...
                                <div className={`w-[4.5rem] h-[4.5rem] flex items-center justify-center rounded-full bg-[#020617] border border-slate-700 ${step.borderHover} group-hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-all duration-500 relative shadow-2xl`}>
                                    
                                    {/* Inner Circle Fill - Reacts on hover */}
                                    <div className={`absolute inset-2 rounded-full bg-slate-900 ${step.bgHover} transition-colors duration-300`}></div>
                                    ...
                                    <div className="absolute -top-1 -right-1 w-6 h-6 bg-slate-200 border border-slate-400 rounded-full flex items-center justify-center shadow-lg">
                                        <span className="font-mono text-[10px] font-bold text-black">0{step.id}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pt-2 md:pt-6 md:px-6 md:text-center max-w-sm">
                                <div className="mb-2">
                                    <h3 className={`font-inter font-black text-xl text-white uppercase tracking-tight group-hover:${step.colorClass} transition-colors duration-300`}>
                                        {step.title}
                                    </h3>
                                </div>
