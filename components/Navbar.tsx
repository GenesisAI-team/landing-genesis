    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-gradient-to-b from-slate-950/90 to-transparent backdrop-blur-[2px]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-24">
          ...
          {/* Logo Section */}
          <div className="flex-shrink-0 flex items-center gap-4 cursor-pointer group z-50 relative">
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="relative group font-pixel text-xs text-slate-300 hover:text-neon transition-colors duration-200 px-3 py-2"
                >
                  <span className="opacity-0 group-hover:opacity-100 absolute left-0 text-neon transition-opacity duration-200">[</span>
                  {link.name}
                  <span className="opacity-0 group-hover:opacity-100 absolute right-0 text-neon transition-opacity duration-200">]</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="-mr-2 flex md:hidden z-50 relative">
            ...
            >
              ...
              {/* Icon toggle handled in CSS/State, but sticking to standard swap here */}
              {isMenuOpen ? (
                 <div className="flex items-center gap-2 px-3 py-1 bg-neon/10 border border-neon rounded-sm">
                    <span className="text-[10px] font-mono font-bold uppercase tracking-widest">CERRAR</span>
        ...
        <div className="fixed inset-0 z-40 bg-[#020617] md:hidden flex flex-col animate-in fade-in duration-200">
            ...
            <div className="flex-grow flex flex-col justify-center px-6 sm:px-8 relative z-10 pt-20">
                ...
                <div className="mb-8 flex items-center gap-2 opacity-50 pl-2">
                    ...
                    <span className="font-mono text-xs text-neon tracking-[0.2em] uppercase">SYSTEM_NAVIGATION // ROOT</span>
                </div>
                ...
                <div className="flex flex-col w-full">
                    {navLinks.map((link, index) => (
                        <a
                            ...
                            className="group relative flex items-center gap-6 py-5 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors duration-300 px-2"
                        >
                ...
                <div className="flex justify-between items-end">
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-inter font-medium text-sm text-slate-300">ONLINE</span>
                        </div>
                    </div>
                    
                    <div className="text-right">
                         <span className="font-pixel text-[10px] text-slate-600 block mb-1">GÉNESIS SYSTEMS</span>
                         <span className="font-mono text-[10px] text-neon tracking-widest">v2.0.24</span>
                    </div>
                </div>
